#PBF3D
#calculate and save particle postion
...
board_states_x = ti.Vector.field(2, float)
board_states_y = ti.Vector.field(2, float)
board_states_z = ti.Vector.field(2, float)
...
ti.root.place(board_states_x)
ti.root.place(board_states_y)
ti.root.place(board_states_z)

@ti.kernel
def move_board_x():
    # probably more accurate to exert force on particles according to hooke's law.
    b = board_states_x[None]
    b[1] += 1.0
    period = 90
    vel_strength = 8.0
    if b[1] >= 2 * period:
    ...
    board_states_x[None] = b

@ti.kernel
def move_board_y():
    # probably more accurate to exert force on particles according to hooke's law.
    b = board_states_y[None]
    b[1] += 1.0
    period = 90
    vel_strength = 8.0
    if b[1] >= 2 * period:
        b[1] = 0
    b[0] += -ti.sin(b[1] * np.pi / period) * vel_strength * time_delta
    board_states_y[None] = b

@ti.kernel
def move_board_z():
    # probably more accurate to exert force on particles according to hooke's law.
    b = board_states_z[None]
    b[1] += 1.0
    period = 90
    vel_strength = 8.0
    if b[1] >= 2 * period:
...
def confine_position_to_boundary(p):
    ...
    bmax = ti.Vector([board_states_x[None][0], board_states_y[None][0], board_states_z[None][0]
...
def init():
    ...
    board_states_x[None] = ti.Vector([boundary[0] - epsilon, -0.0])
    board_states_y[None] = ti.Vector([boundary[1] - epsilon, -0.0])
    board_states_z[None] = ti.Vector([boundary[2] - epsilon, -0.0])
...
if __name__ == '__main__':
    ...
    while gui.running and not gui.get_event(gui.ESCAPE):
        ...
        gui.circles(T(pos)/100.0, radius=particle_radius, color=0x112f41)
        #save particle position for generating mesh
        if(frame >= 300):
            with open("./data/01/xyz/Frame_"+str(frame)+".xyz", "w") as f:
                for p in pos:
                    p[0] /= 40
                    p[1] /= 40
                    p[2] /= 40
